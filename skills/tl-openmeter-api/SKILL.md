...
> See [Subscriptions Deep Dive](references/subscriptions-deep-dive.md) for the full PATCH `customizations` schema (add_item / remove_item), the `stretch_phase` operation, subscription state allowed-operations matrix, and the legacy plan-version migration pattern.
...
> See [Self-Hosted Troubleshooting](references/self-hosted-troubleshooting.md) for the "events not metering" failure mode (Kafka volumes, topic provisioning, restart sequence) and sink-worker partition instability.
