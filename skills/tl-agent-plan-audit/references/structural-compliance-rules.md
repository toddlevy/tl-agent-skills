...
**Step 1 - Determine plan type from the BODY, then check the todos agree.** Classify from what the plan body is: a body with file-level steps, concrete commands, and runnable exit gates is **technical**; a body of milestones and grouped outcomes is **strategic**. Only then read the YAML `todos` array. Technical todo IDs match `t{p}-{g}-{s}` (e.g., `t1-1-1`); strategic IDs are descriptive slugs (e.g., `phase1-foundation`). A technical body whose todos are phase-grained slugs (one todo per phase; 3-6 todos over a body of 15+ steps) is the most common miss and MUST be reported: "VIOLATION 1: Technical body with strategic-grained todos - regranulate to one todo per atomic subtask before audit." The inverse (strategic body with `t{p}-{g}-{s}` ids) is also a violation. If the body itself is ambiguous, report: "VIOLATION 1: Cannot determine plan type." State the classification and how it was derived. (Rationale: classifying from the ids lets a coarse todo list grade itself as compliant; eight technical spokes passed this way on 2026-09-03 and were regranulated after their build had started.)
