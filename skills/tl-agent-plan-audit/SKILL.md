...
- **Dependency addition without lockfile ownership**: a plan adds a package to `dependencies`, `devDependencies`, or `peerDependencies` but omits the repository lockfile from Owned files, so the builder cannot produce the frozen install state the package manifest declares. Probe: inspect the target lockfile importer for the package and require the plan to own and regenerate the lockfile in the same candidate; an already-present package elsewhere in the workspace does not satisfy the changed importer.
...
- **Runtime dependency asserted from an import line**: a plan claims package A "depends on" B because `import ... from 'B'` appears, without distinguishing `import type` (erased at compile, no runtime edge) from a value import. Probe: `rg -n "^import (?!type)" packages/<pkg>/src -g '*.ts' | rg "'B'"` and separately `rg -n "^import type .* from 'B'"`; a type-only hit is not a runtime dependency and any task that adds/moves B on that basis is a false premise (0.152.0 S1e: pg-boss in core/jobs was `import type` ×3).
- **Path shape asserted as a directory when it is a file (or vice versa)**: a plan names `src/<name>` as a folder to split or as a file to rename without probing which it is. Probe: `Test-Path -PathType Container` and `-PathType Leaf` on the exact path and quote the boolean; a wrong shape invalidates every task that enumerates children or rewrites the module (0.152.0 S1g: `query-api` is a single file).
- **Logic located by memory of where it "should" live**: a plan states which module owns a state machine, validator, or default table without a symbol probe, then plans edits against that module. Probe: `rg -n "<distinctive identifier>" <candidate roots>` and quote the `file:line`; when the hit lands elsewhere (a hub gate instead of a package, a lib instead of a CLI), the edit target is a false premise (0.152.0 S1j: the prune state machine lives in `scripts/lib/check-consumer-prune-drift.ts`, not consumer-kit).
- **Contract block names a type or method with no declaration**: a frozen `## Contract` calls a type "canonical" or composes a method (`registry.phaseOf`, `PgSslInput`, `DbPort`) that no owning package declares. Probe: for every identifier in the Contract block, `rg -n "(interface|type|function|const) <Name>\b|\.<method>\(" packages/<owner>/src` and quote the `file:line`; zero hits is a false premise, and the plan must either declare it in an Owned file or drop the reference (0.152.0 S1e `registry.phaseOf`, S1d `PgSslInput`, S1g `DbPort`, S1b "canonical PipelinePhase").
- **Frozen contract exposes a builder without listing every input the builder needs**: a contract shows `buildX()` / `createX()` while its options type omits a value the implementation cannot derive (`sessionTtlDays`, `secure` for an unlock-cookie builder). Probe: read the current implementation's parameter list and diff it against the contract's option fields; an uncovered required input is a false premise, not an "implementation detail" (0.152.0 S1a).
- **Owned-files path never probed for existence**: `## Owned files` lists a directory or file (`src/components/`, `sandbox/src/server/search.ts`) that does not exist and is not marked `(new)`. Probe: `git ls-files -- <path>` count plus `Test-Path -PathType Container|Leaf` for every Owned row and quote both; an unmarked zero is a false premise that silently re-homes the whole spoke (0.152.0 S1g, S1h - second sighting; the hub now also gates it).
- **Shared-library contract audited against the hub only, never against a consumer call site**: a plan adds or changes an exported signature (a schema parameter type, a hook's return type, a peer range, a config object) and the audit checks that the hub declares it but never compiles it against the code that will import it. Probe: for every new or changed export, write a scratch `.ts` under `$env:TEMP` that imports the PROPOSED signature and calls it exactly as each consumer's existing call site does (copy the call site verbatim; use each consumer's own dependency majors - `rg '"zod"|"fastify-plugin"|"eslint-plugin-react-hooks"' <consumer>/**/package.json`), run `tsc --noEmit`, and quote the result per consumer; a red on any consumer is a false premise, and a green on the hub alone is not evidence (0.152.0 wave 3: seven of eighteen post-uplift defects - `loadProcessEnv` typed against zod 3 while three consumers pin zod 4, `createSessionRoleHook` not assignable to `AdminAuthHook`, `fastify-plugin ^5` peer resolving two copies of core under a `^6` plugin, react-hooks `^5` under a v7 consumer, `withDataPort` projecting no context at three call sites, `reconcile --check` vs every consumer's prettier - were each readable from consumer code the audit had on disk).

- **A-family adoption row without a signature-equivalence proof**: a consumer-uplift row says "adopt substrate X, delete bespoke Y" (a `withDataPort` wrapper, a lockdown helper) without a probe showing X's exported signature covers every call site Y served. Probe: `rg -n "<Y>\("` in the consumer and diff each call against X's declared parameters; PJJ / DRTPK / LSC kept domain wrappers at 0.152.0 wave 3 because the row assumed equivalence.
- **Deletion claimed without the replacement named**: a scope row deletes a consumer guard (Primer no-DB guards) without naming the consumer-kit roster entry that replaces it. Probe: `rg -n` the replacement in the kit at the evidence tree; a deletion whose replacement is not on disk is a false premise (0.152.0 wave 3).
- **Bespoke-listener deletion row citing no consumer file:line**: a row deletes "the consumer's MV-refresh listener" without `file:line` proving the listener exists in that consumer. Probe: `rg -n` the listener symbol in the consumer; zero hits means the row deletes nothing and the plan misreads the consumer (0.152.0 S1g).
- **Cohort target derived from an audit approximation, not the metric's own file list**: a prescription row targets a bespoke-LOC floor taken from the audit's estimate rather than from `fleet:bespoke-loc`'s file list. Probe: run the metric and derive the target from its output (0.152.0 wave 3).
- **Writer that lands inside the tree under proof**: a mechanism writes a receipt, proof, digest, or ledger file INTO the tree that it - or a sibling gate in the same ceremony - hashes, so the proof is invalidated by construction. Probe: for every new writer, state its path relative to every tree hash the ceremony takes (`git rev-parse HEAD^{tree}`, a readiness digest, a landing receipt identity) and confirm it is outside or excluded. Third sighting 2026-09-13: a release proof file inside the bump commit tree, an rc receipt likewise, and a consumer ledger under `.vip/` inside the readiness tree - two caught at the end-state gate, one at landing.
- **Tree-identity predicate that forgets the ceremony's own artifacts**: a predicate of the form "diff(receiptedTree, tagTree) is a subset of the bump write set" omits the proof / receipt / digest files the ceremony itself commits, so it can never be true. Probe: enumerate the ceremony's complete write set (`git show --stat` of a real bump or rehearsal commit) and require the predicate to include every path in it. Second sighting 2026-09-13.
- **Promised refusal without a named decision state**: a plan says a mechanism "refuses a second same-minor cut" or "is valid for the next cut only" without naming the exact state it reads to decide - the field, file, tag, or command. That is a design decision left for whoever types first, and a cheap-model builder WILL fill it. Probe every "refuses when / accepts when / valid until" for its decision state and amend before dispatch (2026-09-13: an rc-receipt lifecycle ruled by intent only; the builder invented a `consumed` flag, an `authorizedProductionVersion` field, and a `a history-rewriting amend --amend` before it was stopped).
- **Source-to-plan coverage asserted from keyword routing**: a coverage matrix maps source labels or recommendation families to plans but never proves WHO acts and consumes, WHAT behavior survives, WHEN it runs, which AUTHORITY decides, whether REALISM matches the production seam, how FAILURE stops safely, or which prior behavior the plan REPLACES. Probe every material recommendation through one canonical dimensional matrix with a resolvable source cite, status-valid plan/code/record authority, a plan-only owner ruling, and explicit opposite red/green acceptance. Any missing dimension is a falsified preservation premise, not partial coverage.
- **Windows command shim approved without executing the exact child-process shape**: a plan says Node can spawn a `.cmd` or `.bat` shim with `shell: false`, or otherwise infers Windows command resolution from an interactive shell. Probe the exact executable and argument array with the ruled child-process API on Windows and quote status/error output. A direct `.cmd` spawn returning `EINVAL` is a falsified premise; use a directly spawned `cmd.exe` with fixed `/d /s /c` argv or another proven executable boundary without interpolating user or secret data.
- **Path normalization ruled for a security decision without probing the router side**: a plan normalizes the request path a gate, allowlist, or policy decides on (resolving `.` / `..`, `%2e%2e`, `//`) and the audit checks only that "exempt prefix, then `..`" is now refused. The router still matches the RAW path, so normalizing the decision alone opens the reverse bypass: the decision sees an exempt path while the router dispatches to a protected handler (`/api/x/../../__gate` exempt, served by an `/api/*` route). Probe: feed each non-canonical shape (dot segments, percent-encoded dots, empty segments, protocol-relative, absolute-form, unparseable) through BOTH the decision function and a live router, and quote both outcomes per shape; any shape where the decision exempts and the router reaches a protected handler is a falsified premise. The safe rule is canonicalize-or-refuse: decide only on a path whose raw form already equals its normalized form, and gate everything else (2026-09-24 vip-core lockdown gate, second-round audit).
- **Transport behavior "proven" through an in-process injector that normalizes the request**: a prescription row or ruled test asserts how a server treats a malformed or non-canonical URL, but runs through an injector that rewrites the URL before routing (Fastify `app.inject`, framework test clients). The test passes on code that lets the same bytes through a real connection (`inject` saw `/admin` and got `302`; a raw request for `/__gate/../admin` got `200`). Probe: rerun the row against a listening server (`listen` on port 0 plus a raw `node:http` request carrying the exact `path`) and quote both results; an injector-only proof of URL handling is decorative, not evidence (2026-09-24 vip-core lockdown gate, second-round audit).
